...
public class PhotoProvider
{
	...
	{
		try {
			File cacheDir = AswaqApp.getAppContext().getExternalCacheDir();
			if (cacheDir == null){//cache dir not available
				cacheDir = AswaqApp.getAppContext().getCacheDir();
			}
			File thisCacheDir = new File(cacheDir.getPath(), "Aswaq");
			...
			ImageLoaderConfiguration.Builder builder = new Builder(AswaqApp.getAppContext());
...
//			builder.memoryCache(new UsingFreqLimitedMemoryCache(2000000));
	...
	private void checkLastCacheClearTimestamp()
	{
		try {
			...
			long timenow = AswaqApp.getTimestampNow();
			// compare times
