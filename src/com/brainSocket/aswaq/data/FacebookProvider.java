...
public class FacebookProvider {
	...
	private FacebookCallback<Sharer.Result> shareCallback = new FacebookCallback<Sharer.Result>() {
		...
		public void onError(FacebookException error) {
			if (shareListener != null)
				shareListener.onShareError(AswaqApp.getAppContext().getString(R.string.sharing_failed));
	...
	private FacebookProvider() {
		try {
			...
			FacebookSdk.sdkInitialize(AswaqApp.getAppContext());
...
/*		AppInviteContent gameRequestContent = new AppInviteContent.Builder()
	...
	public void sharePhotoViaFacebook(Activity activity, File filePhoto,
			String caption) {
		try {
			Uri path = Uri.fromFile(filePhoto);
			SharePhoto sharePhoto = new SharePhoto.Builder().setImageUrl(path)
					.build();
			...
			if (ShareDialog.canShow(SharePhotoContent.class)) {
				...
				shareListener.onShareError(activity
						.getString(R.string.message_fb_not_installed));
			}
		} catch (Exception e) {
		}
	}
	
	public void sharePhotoViaFacebook(Activity activity, Bitmap xphoto,
			String caption) {
		try {
			//Uri path = Uri.fromFile(filePhoto);
	...
	public void shareViaFacebook(final Activity activity,
		...
		try {
			...
			if (shareListener != null) {
				shareListener.onShareError(activity.getString(R.string.error_operation_failed));
			}
		}
	}
