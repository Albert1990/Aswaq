...
public class UserPageActivity extends AppBaseActivity implements
		OnClickListener, HomeCallbacks, OnItemClickListener {
	...
	private void init() {
		...
		View llheader = this.getLayoutInflater().inflate(
				R.layout.layout_user_page_header, lvAds, false);
		ivUser = (ImageView) llheader.findViewById(R.id.ivUser);
		tvUserName = (TextViewCustomFont) llheader
				.findViewById(R.id.tvUserName);
		tvFollowers = (TextViewCustomFont) llheader
				.findViewById(R.id.tvFollowers);
		btnFbPage = (ImageView) llheader.findViewById(R.id.btnFbPage);
		tvUserRating = (TextViewCustomFont) llheader
				.findViewById(R.id.tvUserRating);
	...
	private void initCustomActionBar() {
		...
		ivEditUserProfile = (ImageView) mCustomView
				.findViewById(R.id.ivEditUserProfile);
		...
		public void onDataReady(ServerResult data, boolean success) {
			try {
				...
				if (success) {
					...
					AdvertisesListAdapter adsAdapter = new AdvertisesListAdapter(
							getApplicationContext(), userAds);
					lvAds.setAdapter(adsAdapter);
					int followersCount = (Integer) data
							.getValue("followersCount");
					isFollowedByMe = (Integer) data.getValue("isFollowedByMe");
					tvFragTitle.setText(user.getName());
					String photoPath = AswaqApp.getImagePath(ImageType.User,
							user.getPicture());
					PhotoProvider.getInstance().displayPhotoNormal(photoPath,
							ivUser);
					tvUserName.setText(user.getName());
					String followersCountStr = Integer.toString(followersCount)
							+ " "
	...
	private void follow(int userId) {
		...
		DataStore.getInstance().attemptFollowUser(userId, true,
				followUserCallback);
	}
	...
	private DataRequestCallback onRatingCallback = new DataRequestCallback() {
		...
		public void onDataReady(ServerResult data, boolean success) {
			...
			showProgress(true);
			DataStore.getInstance().attemptRateUser(user.getId(), strRating,
					userRateCallback);
	...
	public void onClick(View v) {
		...
		case R.id.tvFollow:
			...
			if (me != null) {
				...
				if (isFollowedByMe == 0)
					...
					DataStore.getInstance().attemptFollowUser(userId, false,
		...
		case R.id.llUserRating:
			me = DataStore.getInstance().getMe();
			if (me != null) {
				DiagRating ratingDialog = new DiagRating(user.getRate(),
						onRatingCallback);
	...
	public void onItemClick(AdapterView<?> parent, View view, int position,
		...
		if (position > 0) {
			...
			if (userAds != null) {
				...
				Intent i = new Intent(UserPageActivity.this,
						AdvertiseDetailsActivity.class);
